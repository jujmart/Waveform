...
function User() {
  ...
  useEffect(() => {
    currentUser.id === user.id ? setCurrentUserProfile(true) : setCurrentUserProfile(false)
  }, [user])
...
{/* List of user created playlists */}
    <div>
      <h2>{currentUserProfile ? "Your playlists" : `${user.username}'s playlists`}</h2>
      <div>
...
{/* List of the users most recently added songs */}
    ...
    <div>
      <h2>{currentUserProfile ? "Your most recently added songs"
                           : `${user.username}'s most recently added songs`}</h2>
      <div>
        <h4>name of song</h4>
        <p>album name</p>
        <p>date added</p>
